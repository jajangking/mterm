...
| **UI dark penuh + status bar + keyboard tap** (2026-09-13) | ✅ statusBarsPadding (teks tidak ketutup statusbar di Android 14 edge-to-edge), theme `darkColorScheme` + surface `0x0B0B0D` (hilangkan layar "setengah hitam/putih" — diverifikasi via analisis piksel screenshot), tap→keyboard via `detectTapGestures` terpisah (kini reliable; sebelumnya cuma onDragEnd yang sering gagal). Diverifikasi jari asli ✓ |
| **Echo dobel hilang** (2026-09-13) | ✅ `TermSession.input()` TIDAK lagi feed engine langsung (`nativeWrite`); tampilan murni dari PTY echo shell. Sebelumnya char tampil 2x (local-echo + PTY echo). Bonus: `stty -echo` / password tidak bocor. Verifikasi device: `hallo123` tampil sekali ✓ (commit echo-fix; **HASH-verified installed == built**) |
| **Pelajaran: signature CI berubah tiap build** | ⚠️ Tiap run CI bikin debug keystore baru → APK beda signature tiap build → `adb install -r` (update) GAGAL `INSTALL_FAILED_UPDATE_INCOMPATIBLE`. WAJIB `adb uninstall com.mterm.app` dulu, atau pakai `./scripts/adb-wireless.sh install` (sudah uninstall+install). Jangan `adb install -r` APK hasil CI. |
| **SGR mouse device — TERBLOCK sampai di sini** | 🚧 gesture jari sampai ke Compose (log `drag-start` muncul) tapi `mouseMode` chrome tidak pernah `true` walau engine dicek langsung via `nativeMouseEnabled()` (poll 100ms) setelah `printf '\033[?1006h'`. Unit test Rust buktikan engine emit `Mouse(true)` pada `?1006h` (terminal.rs:766-771). **Belum dipecahkan.** Hipotesis berikutnya: bikin grid debug nampilin state `mouse_enabled()` biar tes penentu (atau lanjut setup build lokal `build-android.sh` biar loop cepat). |

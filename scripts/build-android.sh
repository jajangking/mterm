...
ci_fetch() {
  # ambil APK dari run CI terbaru (status completed + sukse) untuk branch ini
  local branch="$(cd "$ROOT" && git rev-parse --abbrev-ref HEAD)"
  local outdir="$ROOT/app/app/build/outputs/apk/debug"
  mkdir -p "$outdir"
  echo "▶ cari artifact CI terbaru ($branch)" >&2
  local run_id
  ...
  gh run download -R "$repo" "$run_id" -n mterm-debug-apk -D "$tmpdir" >/dev/null
  local apk; apk="$(find "$tmpdir" -name 'app-debug.apk' | head -1)"
  [ -n "$apk" ] || { echo "artifact app-debug.apk tidak ada di run #$run_id" >&2; exit 1; }
  mkdir -p "$outdir"
  cp -f "$apk" "$outdir/app-debug.apk"
  echo "$outdir/app-debug.apk"
}
